...
var generateFortuneCookie = function() {
  ...
  //random fortune limited by floor and lenght of forutne array
  //random fortune limited by floor and lenght of forutne array
var randomFortune = Math.floor(Math.random()*fortunesList.length); //random fortune limited by floor and lenght of forutne array

  
if ( fortunesList.indexOf(
  (document.getElementById("previous-fortunes-container").lastChild.innerHTML)) ==randomFortune) 
  {document.getElementById("fortune-cookie-text").innerText = "Fortune favors the brave.";
  }  else { document.getElementById("fortune-cookie-text").innerText = (fortunesList[randomFortune]); } 
  //[randomFortune] represents the index of the list
	
	var createNew = document.createElement("li");
  if ( fortunesList.indexOf(
  (document.getElementById("previous-fortunes-container").lastChild.innerHTML)) ==randomFortune) 
  {createNew.innerText = "Fortune favors the brave.";
  }  else { createNew.innerText = (fortunesList[randomFortune]) } 
	
//   createNew.innerText = (fortunesList[randomFortune]);
  
 	document.getElementById("previous-fortunes-container").appendChild(createNew);


  setTimeout(function() {
    createNew.className = createNew.className + " show";
  }, 10);

  twitterString = document.getElementById("previous-fortunes-container").lastChild.innerHTML;
   twitterStringFormat = "http://twitter.com/intent/tweet?text=My+2016+Fortune:"+twitterString.replace(/\s+/g, '+') ;
    document.getElementById('twitter').href=twitterStringFormat
